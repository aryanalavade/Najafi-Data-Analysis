...
def read_camera(ops, z_score=True):
    ...
    # If time vector and pupil area vector different length, set to shorter of the two
    if len(camera_pupil) > len(camera_time):    
        camera_pupil = camera_pupil[:len(camera_time)]
        print("WARNING: FLIR pupil vector > time vector")
    elif len(camera_time) > len(camera_pupil):
        camera_time = camera_time[:len(camera_pupil)]
        print("WARNING: FLIR time vector > pupil vector")
...
def read_neural_trials(ops, smooth):
    ...
    with h5py.File(file_path, 'r') as f:
        ...
        neural_trials['vol_start']    = create_memmap(f['neural_trials']['vol_start'], 'int8',    os.path.join(mm_path, 'vol_start.mmap'))
